...
def index_data(db:Session,skip=0,limit=10,category=None,source=None):
    if category:
        category_data_list=db.execute(f"select id,category,title,img_src,source,keyword from data  where status=1 and keyword = '{category}' ORDER BY create_time DESC LIMIT {skip*limit},{limit}")
        return list(category_data_list)
    elif source:
        return list(db.execute(f'select * from data where source="{source}"'))
    else:
        return list(db.execute(f'select id,category,title,img_src,source,keyword from data  where status=1 ORDER BY create_time DESC LIMIT {skip},{limit}'))

def get_categorys(db:Session,count=8):
    ...
    if len(category_list)>=count:
        ...
        category_list=random.sample(category_list,6)
...
def info_data(db:Session,count,source=None):
    if source:
        return list(db.execute(f'select * from data where source="{source}"'))
    else:
